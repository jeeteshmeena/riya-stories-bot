...
async def extract_story_description(story_name: str, platform_name: str) -> Optional[str]:
    ...
    try:
        resp = await asyncio.to_thread(requests.post, "https://google.serper.dev/search", json=payload, headers=headers, timeout=5.0)
        data = resp.json()
    ...
    for target_url in target_urls[:2]: # try up to 2 pages max
        try:
            html_resp = await asyncio.to_thread(requests.get, target_url, timeout=5.0)
...
async def extract_hd_image(story_name: str, platform_name: str) -> Optional[bytes]:
    ...
    if target_urls:
        try:
            html_resp = await asyncio.to_thread(requests.get, target_urls[0], timeout=5.0)
    ...
    try:
        resp = await asyncio.to_thread(requests.post, "https://google.serper.dev/images", json=payload_img, headers=headers, timeout=5.0)
        data = resp.json()
    ...
    for base_u in img_urls[:2]:
        ...
        for u in variants:
            ...
            try:
                resp = await asyncio.to_thread(requests.get, u, timeout=5.0)
